...
![collections](/images/workflow_diagrams/2_find_funding_account_collections_and_settlements.jpg)
...
Screening Inbound Transactions is an opt-in service that allows you to screen inbound transactions and either accept or reject them. Your decision is required within 23.5 hours.  If no response is received, a default response of ‘accept’ is taken. The decision is then down to our internal screening alone.
Both your response and the result of our internal screening are required before the transaction is processed.  If both parties accept the transaction, then the funds are credited to the beneficiary account. If either party rejects the transaction, then the funds are automatically returned to the original sender.
